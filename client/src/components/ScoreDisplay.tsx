...
export function ScoreDisplay({ score }: ScoreDisplayProps) {
  ...
  useEffect(() => {
    if (score !== null) {
      ...
      const animation = animatedValue.on("change", (latest) => {
        setDisplayValue(Math.round(latest));
      });

      animatedValue.set(0); // reset
      const controlsPromise = animatedValue.set(score, {
        duration: 1.5,
        ease: "easeOut",
      });
  ...
  const getScoreDetails = (s: number | null) => {
    if (s === null) return { color: "text-muted-foreground", circle: "stroke-muted", message: "Ready to calculate" };
    if (s >= 80) return { color: "text-emerald-500", circle: "stroke-emerald-500", message: "Excellent Wealth Health!" };
    if (s >= 50) return { color: "text-blue-500", circle: "stroke-blue-500", message: "Good Financial Baseline" };
    if (s >= 20) return { color: "text-amber-500", circle: "stroke-amber-500", message: "Caution: High Expenses" };
    return { color: "text-red-500", circle: "stroke-red-500", message: "Critical: Expenses exceed safe limits" };
  };
